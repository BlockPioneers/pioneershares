...
{    
    if(block)
	{
        ...
        if (pindex->IsProofOfStake()) { 
            ...
            ui->moneySupplyBox->setText(QString::number(getMoneySupply(height), 'f', 6) + " PIO");
    } 
    
    else {
        std::string txid = ui->txBox->text().toUtf8().constData();
        ui->valueBox->setText(QString::number(getTxTotalValue(txid), 'f', 6) + " PIO"); 
        ...
        ui->feesBox->setText(QString::number(getTxFees(txid), 'f', 6) + " PIO"); 
    }
}

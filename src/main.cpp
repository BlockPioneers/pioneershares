...
const string strMessageMagic = "Pioneershares Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Pioneershares", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Pioneershares genesis - 4th of August 2016";
        CTransaction txNew;
        txNew.nTime = 1470329600;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1470329600;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3720122;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
